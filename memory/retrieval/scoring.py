...
from models.memory import TIER_DECAY_RATE, MemoryTier
...
def _decay_weight(claim: ClaimORM, now: Optional[datetime] = None) -> float:
    ...
    weight = (
        claim.base_importance
        * math.exp(-lam * delta_days)
        * usage_mult
        * confidence_mult
    )
...
def score_claim(
    ...
    usage_score = (
        min(claim.retrieval_hit_count / max(claim.access_count, 1), 1.0)
        if claim.access_count
        else 0.0
    )
